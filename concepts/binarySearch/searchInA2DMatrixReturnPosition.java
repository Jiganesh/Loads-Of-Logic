...
public class searchInA2DMatrixReturnPosition {
    public static void main(String[] args) {
        SolutionSIA2DRP  solution = new SolutionSIA2DRP();
        //System.out.println(Arrays.toString(solution.searchMatrix(new int[][] {{1,3,5,7},{10,11,16,20},{23,30,34,60}}, 3)));
        //System.out.println(Arrays.toString(solution.searchMatrix(new int[][] {{1,3,5,7},{10,11,16,20},{23,30,34,60}},13)));
        //System.out.println(Arrays.toString(solution.searchMatrix(new int [][]{{1,3,5,7},{10,11,16,20},{23,30,34,60}},13)));
        System.out.println(Arrays.toString(solution.searchMatrix(new int[][]{{1},{3}}, 3)));
    }    
}

class SolutionSIA2DRP {

    public int [] binarySearch (int [][]matrix , int row, int cStart, int cEnd, int target){
        System.out.println(cStart+" "+cEnd);
        while (cStart <= cEnd){
            int cMid = cStart + (cEnd -cStart)/2;
            System.out.println(matrix[row][cMid]);
            if (matrix[row][cMid]== target) return new int[]{row, cMid};
    ...
    public int [] searchMatrix(int[][] matrix, int target) {
        ...
        while(rStart<(rEnd-1)){
            ...
            if (matrix[rMid][cMid]== target) return new int []{rStart , cMid};
