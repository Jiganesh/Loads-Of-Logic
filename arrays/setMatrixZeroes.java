...
class SolutionSMZ {
   ...
   public int[][] setZeroesApproach4(int[][] matrix) {
    ...
    public void setZeroesApproach2(int[][] matrix) {
        ...
        for (int row = 0 ; row< matrix.length; row++){
            for(int col=0 ; col < matrix[0].length; col++){
                if (rowTracker[row]==-1 ||colTracker[col]==-1) matrix[row][col]=0;
    ...
    public void setZeroesApproach3(int[][] matrix) {
        ...
        for (int row = 0 ; row< rows; row++){
            for(int col=0 ; col < cols; col++){
                ...
                // checking if any zero in any row and changing boolean inRow so we can change whole row to zero
                if (row ==0 && matrix[0][col]==0) inRow = true;
                // checking if any zero in any col and changing boolean inCol so we can change whole col to zero
                if (col ==0 && matrix[row][0] ==0) inCol = true;
        ...
        for (int row =rows-1; row>=0; row--){
            for(int col = cols-1; col>=0; col--){
                ...
                // Traversing from back so we do not make any change to stored values in row =0 and col =0
                // as we are using them to keep track
                if (inRow && row==0) matrix[0][col]=0;
                if (inCol && col==0) matrix[row][0]=0;
